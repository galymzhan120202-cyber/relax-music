...
`.github/workflows/main.yml` ішінде:

```yaml
schedule:
  - cron: "0 15 * * *"   # 20:00 Almaty (UTC+5) = 15:00 UTC
```

GitHub Actions cron әрдайым **UTC** уақытпен жұмыс істейді. Егер басқа
уақытта жіберу керек болса, `cron` мәнін өзгертіңіз (UTC-ке қайта
есептеп).
...
Cron іске қосылғаннан кейін жүйе күн сайын белгіленген уақытта өздігінен:
