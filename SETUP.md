...
- **SEO**: Google Gemini API арқылы атау/сипаттама/хэштег
...
- (Міндетті емес) Gemini API кілті — SEO атау/сипаттама үшін. Болмаса, скрипт үлгі мәтінді қолданады.
...
## 3. (Міндетті емес) Gemini API кілті

SEO атау/сипаттама/хэштегтерді Google Gemini арқылы генерациялау үшін
(әр видеоға кездейсоқ эмоционалды акцент таңдалып, күн сайын
қайталанбайтын мәтін жазылады):

1. https://aistudio.google.com/apikey бетінде тегін API кілт жасаңыз.
2. Бұл — `GEMINI_API_KEY` секреті.
...
| `GEMINI_API_KEY` | (міндетті емес) 3-қадамдағы Gemini кілті |
...
4. Gemini арқылы (немесе fallback үлгімен) SEO атау/сипаттама жасайды,
